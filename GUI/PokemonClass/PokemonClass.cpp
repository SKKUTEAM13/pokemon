...
string Pokemon::getSkill(int i) { return skillSeries[i - 1]; } // 포켓몬이 가지고 있는 i 번째 스킬 이름 반환. 
...
int Pikachu::FirstSkill(string skillName)  // 첫번째 스킬 기본 데미지와 이름 반환
{
	srand(time(NULL));
...
int Pikachu::SecondSkill(string skillName) // 두번째 스킬 기본 데미지와 이름 반환
{
	srand(time(NULL));
